...
df_capita_solar <- read.csv("./datasets/per-capita-solar.csv",header=TRUE)
# Solar PV module prices
df_price <- read.csv("./datasets/solar-pv-prices.csv",header=TRUE)
# Share of electricity production from solar
df_share <- read.csv("./datasets/share-electricity-solar.csv",header=TRUE)
# Installed solar energy capacity
df_capacity <- read.csv("./datasets/installed-solar-PV-capacity.csv",header=TRUE)
# Per capita energy consumption from nuclear
df_capita_nuclear <- read.csv("./datasets/per-capita-nuclear.csv",header=TRUE)
# Per capita energy consumption from wind
df_capita_wind <- read.csv("./datasets/per-capita-wind.csv",header=TRUE)
# Per capita energy consumption from fossil
df_capita_fossil <- read.csv("./datasets/fossil-fuels-per-capita.csv",header=TRUE)
# Per capita energy consumption from oil
df_capita_oil <- read.csv("./datasets/per-capita-oil.csv",header=TRUE)
# Per capita energy consumption from gas
df_capita_gas <- read.csv("./datasets/per-capita-gas.csv",header=TRUE)
# Per capita energy consumption from coal
df_capita_coal <- read.csv("./datasets/coal-consumption-per-capita.csv",header=TRUE)
# solar power generation
df_solar_generation <- read.csv("./datasets/solar-energy-consumption.csv",header=TRUE)
...
legend("topright", legend=c("Solar", "Wind", "Nuclear", "Coal", "Gas","Oil"),
       col=c(7,4,3,1,6,2),  lty = 1, cex=1.5, lwd = 3)
dev.off()
...
plot(df2$Year, df2$Share.from.solar, type = "o", col = 7, xlab="Year", ylab = "Share (%)", main ="Share of electricity production from solar", lwd = 1.5, cex.lab = 1.5, cex.main = 1.8, pch = 16)
dev.off()

```


# Start with the LM model of per capita solar enegy consumption in Spain.
...
res_BM <- acf(res_BM_SP)

plot(res_BM, xlab = "Lag", main = "Autocorrelation of residuals", col = 7, lwd = 4, ci.col = 7, cex.lab = 1.5, cex.main = 1.8)
...
res_GBM <- acf(res_GBM_SP)

plot(res_GBM, xlab = "Lag", main = "Autocorrelation of residuals", col = 7, lwd = 4, ci.col = 7, cex.lab = 1.5, cex.main = 1.8)
...
plot(res_GGM_SP)
res_GGM <- acf(residuals(GGM_SP))

plot(res_GGM, xlab = "Lag", main = "Autocorrelation of residuals", col = 7, lwd = 4, ci.col = 7, cex.lab = 1.5, cex.main = 1.8)
...
plot(x = c(1989:2021), res_GGM_SP,lwd=2, col=7, type= "l", xlab="Year", ylab="Residuals (kWh)", pch=16, lty=1, cex.lab=1.5, ylim=c(-500,500) )
lines(x = c(1989:2021), res_GBM_SP, lwd=2, col=4)
lines(x = c(1989:2021), res_BM_SP, lwd=2, col=2)
legend("topleft", legend=c("BM", "GBM with two exp. shocks", "GGM"),
       col=c(2,4,7),  lty = 1, cex=1.5, lwd = 3)
...
# Do forecast
png(file="plots/autoarima.png", width=14,height=12,units="cm",res=1200, pointsize=6)
plot(forecast(auto.a, 5), xaxt='n', xlab="Year", ylab="Per capita consumption (kWh)", type= "b", pch=16, lty=3, cex.lab =1.5, main = " ")
axis(1, at = seq(1, 38, by=5), labels=seq(1989, 2026, by=5))
lines(fitted(auto.a)[1:34], col=7, lwd = 2)
dev.off()
...
#Check residuals
checkresiduals(armax_all_countries)
# Residuals
...
png(file="plots/autoarima_xreg_countries.png", width=14,height=12,units="cm",res=1200, pointsize=6)
plot(df$Year[25:57], cap_solar_SP, type= "b", xlab="Year", ylab="Per capita consumption (kWh)", pch=16, lty=3, cex.lab = 1.5)
lines(df$Year[25:57],fitted_all_countries,  lwd=2,col = 7)
dev.off()
...
xreg <- cbind(Germany=df$Capita.solar.germany[25:54],
              ...
              Australia=df$Capita.solar.australia[25:54],
...
xreg_forecast <- cbind(Germany=df$Capita.solar.germany[54:57],
                 ...
                 Australia=df$Capita.solar.australia[54:57],
                 Price = df$PV.price[54:57],
                 Capacity = df$P4V.capacity[54:57],
                 Share = df$Share.from.solar[54:57])


# Do forecast
png(file="plots/autoarima_xreg_all.png", width=14,height=12,units="cm",res=1200, pointsize=6)
plot(forecast(armax_all, xreg = xreg_forecast), xaxt='n', type= "b", xlab="Year", ylab="Per capita consumption (kWh)", pch=16, lty=3, cex.lab = 1.5,main = "")
lines(fitted_all, lwd = 2, col = 7, cex.lab = 1.5)
axis(1, at = seq(1, 34, by=5), labels=seq(1989, 2022, by=5), cex.lab = 1.5)
dev.off()
...
lines(xxx,fit3,col=4, lwd = 2)
lines(p2, col=7, lwd = 2)
...
SS_N <- df$Capita.solar.spain[25:56]
SN <- df$Capita.nuclear[4:56]
...
ucrcdNS <-UCRCD(SN,SS_N, display=T, m2 = 12431) #the only one that gives me error
summary(ucrcdNS)

png(file="plots/nuclear_vs_solar.png", width=14,height=12,units="cm",res=1200, pointsize=6)
plot(capita_solar_spain$Year[4:56], SN, xlab="Year", ylab="Consumption per capita (kWh)", type="b",pch=16, lty=3, cex.lab=1.5, col = 1)
points(df$Year[25:56], SS_N, type= "b", pch=16, lty=3, col=7)
lines(df$Year[4:56], (ucrcdNS$fitted.i[[1]]), lwd=2, col=1)
lines(df$Year[25:56], (ucrcdNS$fitted.i[[2]]), lwd=2, col=7)
abline(v=1989, lty=2)
...
plot(df$Year[25:57], SW, xlab="Year", ylab="Consumption per capita (kWh)", type="b",pch=16, lty=3, cex.lab=1.5, col = 1)
points(df$Year[25:57], SS, type= "b", pch=16, lty=3, col=7)
lines(df$Year[25:57], (ucrcdWS$fitted.i[[1]]), lwd=2, col=1)
lines(df$Year[25:57], (ucrcdWS$fitted.i[[2]]), lwd=2, col=7)
...
dev.off()
# Test autocorrelation of residuals with durbin watson test
dwtest(gamscope)
