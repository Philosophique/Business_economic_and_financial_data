...
plot(df$Year[25:57], res_ml,xlab="Time", ylab="residuals", type= "b",  pch=16, lty=3, cex=0.6)
...
plot(for1, xaxt = 'n')
axis(1, at=1:38, labels=c(1989:2026))
...
auto_with_reg <- auto.arima(cap_solar_SP, xreg = fit_GBM_SP)
auto_with_reg
summary(auto_with_reg)

plot(df$Year[25:57],cap_solar_SP, type= "b",xlab="Quarter", ylab="Quarterly revenues",  pch=16, lty=1, cex=0.6, col = 1)
lines(df$Year[25:57], fit_GBM_SP_inst, lwd=1, lty=1, col = 2)
lines(df$Year[25:57], fitted(auto_with_reg), lty=1,lwd=1, col=7)
legend("topleft", legend=c("Data", "GGM fit", "Auto ARIMA with xreg"),
...
```{r china influence on cap_solar_SP, results=}
...
xreg <- cbind(Germany=df$Capita.solar.germany[25:57], 
              ...
              Australia=df$Capita.solar.australia[25:57])
...
# Do forecast
forecast(armax_all_countries, 5, xreg = xreg)
plot(forecast(armax_all_countries, 5, xreg = matrix(xreg, ncol=20)), xaxt='n')
axis(1, at=1:38, labels=c(1989:2026))
...
# # Do forecast
forecast(armax_all, 5, xreg = xreg)
plot(forecast(armax_all, 5, xreg = xreg), xaxt='n')
# axis(1, at=1:38, labels=c(1989:2026))
