...
plot(df2$Year, df2$Capita.solar.spain, type = "o", col = 7, ylim = c(1,25000), xlab="Year",ylab = "Consumption (kWh)", main ="Per capita solar energy consumption", cex.lab = 1.5, cex.main = 1.8, pch = 16)
...
##plot of the model
plot(df$Year[25:57], cap_solar_SP, xlab="Time", ylab="Solar per capita kWh", type= "b",  pch=16, lty=3, cex=0.6)
abline(m_lm, col=3)
...
plot(df$Year[25:57], res_lm, xlab="Time", ylab="residuals", type= "b",  pch=16, lty=3, cex=0.6)
...
plot(x = df$Year[25:57], cap_solar_SP, type= "o",xlab="Year", ylab="Consumption (kWh)", main = "Per capita solar energy consumption - model comparison",  pch=16, lty=1, cex=0.6, xlim=c(1989,2023), ylim=c(0,2700))
...
legend("topleft", legend=c("Data", "BM", "GBM with two exp shocks", "GGM"),
       col=c(1,2,4,7),  lty = 1, cex=1.5, lwd = 3)
...
cap_solar_SP <- df$Capita.solar.spain[25:57] # erase this later, it's repeated
...
Own Arima, delete? First solve problem with autoarima (0,2,0)
...
plot(df$Year[25:57],cap_solar_SP, type= "o",xlab="Quarter", ylab="Quarterly revenues",  pch=16, lty=1, cex=0.6, col = 1)
lines(df$Year[25:57], pred_GGM_SP.inst[1:33], lwd=1, lty=1, col = 2)
lines(df$Year[25:57], fitted(auto_with_reg), lty=1,lwd=1, col=7)
legend("topleft", legend=c("Data", "GGM", "Auto ARIMA with xreg"),
       col=c(1,2,7),  lty = 1, cex=1.5, lwd = 3)

plot(forecast(auto_with_reg, xreg = pred_GGM_SP.inst[34:39]), xaxt='n')
...
plot(df$Year[25:57],cap_solar_SP, type= "o",xlab="Quarter", ylab="Quarterly revenues",  pch=16, lty=1, cex=0.6, col = 1)
lines(df$Year[25:57], pred_GBM_SP_e2.inst[1:33], lwd=1, lty=1, col = 2)
lines(df$Year[25:57], fitted(auto_with_reg), lty=1,lwd=1, col=7)
legend("topleft", legend=c("Data", "GBM", "Auto ARIMA with xreg"),
       col=c(1,2,7),  lty = 1, cex=1.5, lwd = 3)

plot(forecast(auto_with_reg, xreg = pred_GBM_SP_e2.inst[34:35]), xaxt='n')
axis(1, at = seq(1, 34, by=5), labels=seq(1989, 2022, by=5))
...
Acf(res_all_countries)

fitted_all_countries <- fitted(armax_all_countries)

plot(df$Year[25:57], cap_solar_SP, type= "b", pch=16, lty=3, cex=0.6)
lines(df$Year[25:57],fitted_all_countries, col = 2)
...
plot(df$Year[25:54], cap_solar_SP[1:30], type= "b", pch=16, lty=3, cex=0.6)
lines(df$Year[25:54],fitted_all, col = 2)
...
# Do forecast
plot(forecast(armax_all, xreg = xreg_forecast), xaxt='n')
axis(1, at = seq(1, 34, by=5), labels=seq(1989, 2022, by=5))
...
plot(df$Year[25:57], cap_solar_SP, type= "b", pch=16, lty=3, cex=0.6)
lines(df$Year[25:57],fitted_all, col = 2)
...
cor_spain_vs_all #Portugal with 0.9449656, Belgium 0.9423068 and Australia 0.9409159 are the country with higher Pearson correlation. Lets look at cross-correlation function (using lagged values of these countries to predict current value of Spain)

ccf_spain_portugal <- ccf(df_capacity_transformed$Portugal, df_capacity_transformed$Spain,10) 
ccf_portugal_spain <- ccf(df_capacity_transformed$Spain, df_capacity_transformed$Portugal,10)

ccf_spain_belgium <- ccf(df_capacity_transformed$Belgium, df_capacity_transformed$Spain,10)
...
#plotting Cross correlation function of every countries againts spain to see if we can see if some countries speding leads to Spain spending. I am not able to plot with the columns.
...
```{r Study different energy consumption plot together, results=}

# Show local regression, regression spline, smoothing spline in one plot

plot(x,y,xlab="Capacity (GW)", ylab="Price ($/W)", main = "Modelling price of PV cells as a function of their capacity")
sm.regression(x, y,   h = 3, add = T, ngrid=200, col=2)
lines(xxx,fit3,col=3)
lines(p2, col=4)
...
df$mean_energy_capita <-  apply(df[,5:11],1,mean) #this i don't get it # I would not do this
...
S_all_energy <- df$mean_energy_capita[1:57] #THIS IS NULL, HAVE TO CHECK # I would not do this
...
#plot of two series
plot(df$Year[1:57], SN, xlab = "Year", ylab="Consumption per capita",type= "b", pch=16, lty=3, cex=0.7)
points(df$Year[25:57], SS, type= "b", pch=16, lty=3, cex=0.7, col=2)

ucrcdNS <-UCRCD(SN,SS, display=T, par = "unique", m2 = 12431) #the only one that gives me error
summary(ucrcdNS)
...
abline(v=1989, lty=2) #THIS I THINK DOESNT MAKE SENSE
...
# Residual analysis
tsdisplay(residuals(gamscope))
aar1<- auto.arima(residuals(ggamscope))

plot(as.numeric(Capita.solar.spain), type="l")
...
# Modelling country competition 



```{r Competition spain vs Germany, China, USA, results=}
### Study of consuption of energy solar per capita of Spain vs Germany, China, USA
